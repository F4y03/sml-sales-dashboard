...
sheet.columns.forEach((column, index) => { column.width = index === 4 ? 48 : [8, 9, 30].includes(index) ? 60 : 24; });
...
const rows = Array.from({ length: sheet.rowCount }, (_, row) => Array.from({ length: sheet.columnCount }, (_, column) => String(sheet.getCell(row + 1, column + 1).value ?? '')));
...
const smlPool = new pg.Pool({ connectionTimeoutMillis: 5000, statement_timeout: 15000, max: 2, options: '-c default_transaction_read_only=on' });
smlPool.on('error', error => console.error('Idle SML connection:', error.code));
...
function workbookSkus(tab) {
  const headers = Array.from({ length: tab.columnCount }, (_, column) => tab.getCell(1, column + 1).text.trim());
...
function workbookHeaders(tab) {
  return Array.from({ length: tab.columnCount }, (_, column) => tab.getCell(1, column + 1).text.trim());
}

app.get('/api/missing-products', async (_req, res) => {
  res.set('Cache-Control', 'no-store');
  if (!existsSync(driveWorkbookPath)) return res.status(503).json({ error: 'ไม่พบไฟล์ Excel สำหรับเปรียบเทียบ' });
  let client;
  try {
    ...
    res.json({ start: missingProductStart, end: new Date().toISOString().slice(0, 10), count: unique.size, rows: [...unique.values()], updatedAt: new Date().toISOString() });
  } catch (error) {
    if (client) await client.query('ROLLBACK').catch(() => {});
    console.error('Missing product query failed:', error.code || error.message);
    res.status(503).json({ error: 'ตรวจสอบสินค้าใหม่จาก SML ไม่สำเร็จ กรุณาลองใหม่' });
  } finally { client?.release(); }
});

let savingProduct = false;
app.post('/api/missing-products', async (req, res) => {
  if (savingProduct) return res.status(429).json({ error: 'กำลังบันทึกสินค้า กรุณาลองใหม่อีกครั้ง' });
  const sourceCode = typeof req.body?.sourceCode === 'string' ? req.body.sourceCode.trim() : '';
  const values = Array.isArray(req.body?.values) ? req.body.values : null;
  if (!sourceCode || !values || values.some(value => typeof value !== 'string') || values.reduce((sum, value) => sum + value.length, 0) > 200000) {
  ...
  try {
    ...
    if (!source.rowCount) return res.status(400).json({ error: 'ไม่พบรหัสสินค้านี้ในช่วงวันที่ที่กำหนด' });
    const workbook = await readDriveWorkbook();
    const tab = workbook.worksheets[0];
    const headers = workbookHeaders(tab);
    if (values.length !== headers.length) return res.status(400).json({ error: 'จำนวนคอลัมน์สินค้าไม่ตรงกับไฟล์ Excel' });
    const skuColumn = headers.indexOf('รหัสสินค้า');
    if (skuColumn < 0) throw new Error('SKU_COLUMN_NOT_FOUND');
    if (values[skuColumn].trim().toLocaleLowerCase() !== sourceCode.toLocaleLowerCase()) return res.status(400).json({ error: 'รหัสสินค้าไม่ตรงกับข้อมูลจาก SML' });
    if (workbookSkus(tab).has(sourceCode.toLocaleLowerCase())) return res.status(409).json({ error: 'รหัสสินค้านี้มีอยู่ใน Excel แล้ว' });
    ...
    res.status(500).json({ error: 'เพิ่มสินค้าลง Excel ไม่สำเร็จ กรุณาลองใหม่' });
  } finally {
...
app.get('/api/sheet', async (req, res, next) => {
  ...
  try {
    const tab = (await readDriveWorkbook()).worksheets[0];
    const values = Array.from({ length: tab.rowCount }, (_, row) => Array.from({ length: tab.columnCount }, (_, column) => tab.getCell(row + 1, column + 1).text));
...
const server = app.listen(3002, '127.0.0.1', () => console.log('Product sheet: http://localhost:3002'));
process.on('SIGINT', () => server.close(async () => { await smlPool.end(); process.exit(0); }));
