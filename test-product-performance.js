...
test('product view: search, rankings, no-sales, quantities, buyers popup, filters, errors, races and mobile',async()=>{
  ...
  try{
    ...
    await page.locator('#performance-search').fill('sku001');
    await page.locator('#performance-search-button').click();
    await expect(page.locator('#sku-detail')).toBeVisible();
    await expect(page.locator('#sku-content')).toBeVisible();
    await expect(page.locator('#sku-subtitle')).toContainText('SKU-SKU001');
    await expect(page.locator('#sku-stock')).toHaveText('25 ชิ้น');
    await page.locator('#sku-close').click();
    await expect(page.locator('#performance-search-button')).toBeFocused();
    await page.locator('#performance-search').fill('sku-SKU002');
    await expect(page.locator('#performance-rows .performance-product-button')).toHaveCount(1);
    await page.locator('#performance-search').press('Enter');
    await expect(page.locator('#sku-detail')).toBeVisible();
    await expect(page.locator('#sku-subtitle')).toContainText('SKU-SKU002');
    await expect(page.locator('#sku-content')).toBeVisible();
    await expect(page.locator('#sku-stock')).toHaveText('24 ชิ้น');
    await page.keyboard.press('Escape');
    await page.locator('#performance-search').fill('missing-sku');
    await page.locator('#performance-search-button').click();
    await expect(page.locator('#sku-detail')).toBeHidden();
    await expect(page.locator('#performance-search-message')).toContainText('ไม่พบสินค้า');
    await page.locator('#performance-search').fill('SKU0');
    await page.locator('#performance-search-button').click();
    await expect(page.locator('#sku-detail')).toBeHidden();
    await expect(page.locator('#performance-search-message')).toContainText('18');
    await page.locator('#performance-search').fill('');
    await expect(page.locator('#performance-best .ph-toggle')).toHaveCount(5);await expect(page.locator('#performance-watch .ph-toggle')).toHaveCount(5);
    // Best sellers: bar length vs rank 1; summary boxes use the total of every product in scope (16,950).
    const bestWidths = await page.locator('#performance-best .ph-fill').evaluateAll(bars => bars.map(bar => parseFloat(bar.style.width)));
    assert.equal(bestWidths[0],100); assert.ok(Math.abs(bestWidths[1]-1700/1800*100)<0.001);
    await expect(page.locator('#ph-best-sum')).toHaveText('฿8,000.00');await expect(page.locator('#ph-best-share')).toHaveText('47.2%');
    await expect(page.locator('#performance-best .ph-item').first()).toContainText('10.6% ของยอดขาย');
    await expect(page.locator('#performance-best .ph-rank.is-top1')).toHaveCount(1);
    await expect(page.locator('#performance-best strong').first()).toHaveAttribute('title',products[0].name);
    // Products to watch: each row uses its own scale (previous = 100%, current = share of previous).
    const watchWidths = await page.locator('#performance-watch .ph-pair-line.is-previous .ph-fill').evaluateAll(bars => bars.map(bar => parseFloat(bar.style.width)));
    const currentWidths = await page.locator('#performance-watch .ph-pair-line.is-current .ph-fill').evaluateAll(bars => bars.map(bar => parseFloat(bar.style.width)));
    assert.deepEqual(watchWidths,[100,100,100,100,100]);assert.deepEqual(currentWidths,[50,50,50,50,50]);
    await expect(page.locator('#performance-watch .ph-pair-line.is-previous').first()).toContainText('ช่วงก่อน฿3,600.00');
    await expect(page.locator('#performance-watch .ph-item').first()).toContainText('−฿1,800.00');
    await expect(page.locator('#performance-watch .ph-item').first()).toContainText('ลดลง 50.0%');
    await expect(page.locator('#performance-watch .ph-level.is-warning')).toHaveCount(5);
    await expect(page.locator('#ph-watch-lost')).toHaveText('−฿7,000.00');await expect(page.locator('#ph-watch-critical')).toHaveText('0 จาก 5 รายการ');
    assert.ok(!/NaN|Infinity/.test(await page.locator('.performance-highlights').innerText()));
    // TOP 5 / TOP 10 segmented control.
    await page.locator('[data-ph-list="best"][data-ph-limit="10"]').click();
    await expect(page.locator('#performance-best .ph-toggle')).toHaveCount(10);
    await expect(page.locator('[data-ph-list="best"][data-ph-limit="10"]')).toHaveAttribute('aria-pressed','true');
    await expect(page.locator('#performance-watch .ph-toggle')).toHaveCount(5);
    await page.locator('[data-ph-list="watch"][data-ph-limit="10"]').click();await expect(page.locator('#performance-watch .ph-toggle')).toHaveCount(10);
    await page.locator('[data-ph-list="best"][data-ph-limit="5"]').click();await page.locator('[data-ph-list="watch"][data-ph-limit="5"]').click();
    await expect(page.locator('#performance-best .ph-toggle')).toHaveCount(5);
    // Expand a best seller: top 3 buyers (names escaped), "and x more", average per bill; one open row per card.
    const firstBest = page.locator('#performance-best .ph-toggle').first();
    await firstBest.click();await expect(firstBest).toHaveAttribute('aria-expanded','true');
    await expect(page.locator('#performance-best .ph-item.is-open .ph-buyers li')).toHaveCount(3);
    await expect(page.locator('#performance-best .ph-item.is-open .ph-detail-more')).toHaveText('และอีก 9 ราย');
    await expect(page.locator('#performance-best .ph-item.is-open')).toContainText('เฉลี่ยต่อบิล฿150.00');
    assert.equal(await page.locator('.performance-highlights svg').count(),0);
    await page.locator('#performance-best .ph-toggle').nth(1).click();
    await expect(firstBest).toHaveAttribute('aria-expanded','false');await expect(page.locator('#performance-best .ph-item.is-open')).toHaveCount(1);
    await page.locator('#performance-best .ph-toggle').nth(1).press('Enter');await expect(page.locator('#performance-best .ph-item.is-open')).toHaveCount(0);
    await firstBest.press('Space');await expect(page.locator('#performance-best .ph-item.is-open .ph-buyers li')).toHaveCount(3);
    await page.locator('#performance-best .ph-item.is-open .ph-link').click();
    await expect(page.locator('#sku-detail')).toBeVisible();await expect(page.locator('#sku-subtitle')).toContainText('SKU-SKU001');
    await page.keyboard.press('Escape');await expect(page.locator('#sku-detail')).toBeHidden();
    // Expand a product to watch: remaining %, lost amount, register stock (NULL stock is not zero).
    await page.locator('#performance-watch .ph-toggle').first().click();
    const openWatch = page.locator('#performance-watch .ph-item.is-open');
    await expect(openWatch).toContainText('50.0% ของช่วงก่อน');await expect(openWatch).toContainText('25 ชิ้น');
    await expect(page.locator('#performance-best .ph-item.is-open')).toHaveCount(1);
    await mkdir('test-results',{recursive:true});
    await page.locator('.performance-highlights').screenshot({path:'test-results/product-leader-charts.png'});
    await page.locator('#ph-watch-all').click();await expect(page.locator('#performance-table-title')).toHaveText('สินค้าที่มียอดลดลง');
    await page.locator('#ph-best-all').click();await expect(page.locator('#performance-table-title')).toHaveText('สินค้าทั้งหมด');
    ...
    // Current period 0: bar keeps a hairline (width 0% + CSS min-width), level is critical, share of total is "–".
    await expect(page.locator('#performance-watch .ph-pair-line.is-current .ph-fill')).toHaveAttribute('style',/width: 0%/);
    assert.ok((await page.locator('#performance-watch .ph-pair-line.is-current .ph-fill').evaluate(bar=>bar.getBoundingClientRect().width))>=2);
    await expect(page.locator('#performance-watch .ph-pair-line.is-current')).toContainText('ช่วงนี้฿0.00');
    await expect(page.locator('#performance-watch .ph-level.is-critical')).toHaveText('วิกฤต');await expect(page.locator('#ph-watch-critical')).toHaveText('1 จาก 1 รายการ');
    await page.locator('#performance-search').fill('NEG');
    await expect(page.locator('#performance-net')).toHaveText('฿-100.00');
    await expect(page.locator('#performance-watch .ph-pair-line.is-current .ph-fill')).toHaveAttribute('style',/width: 0%/);
    await expect(page.locator('#performance-watch .ph-item')).toContainText('ลดลง 200.0%');
    await expect(page.locator('#performance-watch .ph-pair-line.is-current')).toContainText('ช่วงนี้฿-100.00');
    await expect(page.locator('#ph-best-sum')).toHaveText('–');await expect(page.locator('#performance-best .ph-empty')).toBeVisible();
    assert.ok(!/NaN|Infinity/.test(await page.locator('.performance-highlights').innerText()));
    ...
    await page.locator('#performance-rows .performance-product-button').first().click();await expect(page.locator('#sku-content')).toBeVisible();
    ...
    heldCode='SKU001';await page.locator('#performance-rows .performance-product-button').first().click();await expect(page.locator('#sku-detail')).toHaveAttribute('aria-busy','true');
    await expect.poll(()=>Boolean(heldRoute)).toBe(true);
    await page.keyboard.press('Escape');await page.locator('#performance-rows .performance-product-button').nth(1).click();await expect(page.locator('#sku-content')).toBeVisible();
    ...
    failBuyers=true;await page.locator('#performance-rows .performance-product-button').first().click();await expect(page.locator('#sku-retry')).toBeVisible();await expect(page.locator('#sku-content')).toBeHidden();
    ...
    await page.locator('#performance-rows .performance-product-button').first().click();await expect(page.locator('#sku-content')).toBeVisible();assert.equal(requests.at(-1).searchParams.get('start'),'2026-08-01');await page.keyboard.press('Escape');
