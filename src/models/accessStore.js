...
export function createAccessStore(path = ':memory:', env = {}) {
  ...
  for (const file of ['001-access.sql','002-security.sql','003-product-images.sql','004-product-image-imports.sql','005-unmatched-product-images.sql']) db.exec(readFileSync(new URL('../../migrations/'+file, import.meta.url), 'utf8'));
  const store = {
    db, path,
