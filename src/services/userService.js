...
export function createUserService(store,audit) {
  const get=id=>{const u=loadUser(store,id);if(!u)throw bad('ไม่พบผู้ใช้',404);return {...u,territoryIds:store.all('SELECT territory_id FROM user_territories WHERE user_id=?',id).map(t=>t.territory_id),twoFactorRequired:true};};
