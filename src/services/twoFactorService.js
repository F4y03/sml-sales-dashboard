...
export const CHALLENGE_MS=10*60*1000,TRUST_MS=30*24*60*60*1000,RECOVERY_COUNT=10;
const digest=value=>createHash('sha256').update(value).digest('hex');
const normalizeRecovery=code=>String(code).replace(/[\s-]/g,'').toUpperCase();
const formatRecovery=raw=>raw.match(/.{4}/g).join('-');
// 2FA is mandatory for Super Admin only. AUTH_REQUIRE_2FA=false is a development/test escape hatch, never for production.
export function createTwoFactorService(store,audit,env=process.env) {
  ...
  const service={
    required:user=>enforced&&user.role==='super_admin',
    isEnrolled,
    ...
    isTrusted:(userId,token)=>!!token&&/^[a-f0-9]{64}$/.test(token)&&!!store.get('SELECT 1 FROM trusted_devices WHERE token_hash=? AND user_id=? AND expires>?',digest(token),userId,Date.now()),
