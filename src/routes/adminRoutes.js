...
export function installAdminRoutes(app,store,audit,envPath,twofa=createTwoFactorService(store,audit)) {
  ...
  app.post('/api/admin/users/:id/2fa/reset',requireSuperAdmin,handle((req,res)=>{const id=positiveId(req.params.id);if(id===req.auth.id)throw bad('ต้องให้ Super Admin คนอื่นรีเซ็ต 2FA ของบัญชีนี้',403);if(!store.get('SELECT 1 FROM users WHERE id=?',id))throw bad('ไม่พบผู้ใช้',404);store.transaction(()=>{twofa.reset(id);store.run('DELETE FROM sessions WHERE user_id=?',id);audit.record(req.auth,'2fa.reset','security',{userId:id},null,req.socket.remoteAddress);});res.json({ok:true});}));
