...
test('monthly endpoint validates years and chart switches between daily and monthly with retry and themes', async () => {
  const app = express(); let failed = false; const dailyCalls = [];
  installSalesTrend(app, { query: async (sql, params) => {
    ...
    if (sql.includes("to_char(s.day, 'YYYY-MM-DD')")) {
      dailyCalls.push(params);
      return { rows: [{ day: params[0], sales: 40 }, { day: params[1], sales: 60 }] };
    }
    if (failed) throw Object.assign(new Error('offline'), { code: 'TEST_OFFLINE' });
    if (params[0] !== '2025-01-01') return { rows: Array.from({ length: 12 }, (_, i) => ({ month: i + 1, sales: 0 })) };
  ...
  try {
    ...
    await page.locator('[data-trend-mode="monthly"]').click();
    await expect(page.locator('#monthly-status')).toContainText('76,000');
    await expect(page.locator('#trend-title')).toHaveText('แนวโน้มยอดขายรายเดือน');
    assert.deepEqual(await page.locator('#trend-year option').evaluateAll(options => options.map(o => o.textContent)), ['2568', '2567']);
    assert.deepEqual(await page.evaluate(() => window.salesTrend.debug().monthly.values), [1000,0,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000]);
    // A month without sales has no bar; the other 11 months do.
    await expect(page.locator('#monthly-chart .tg-bar')).toHaveCount(11);
    await expect(page.locator('#daily-trend-wrap')).not.toBeVisible();
    for (const mode of ['light','dark']) {
      await page.locator(`[data-theme-mode="${mode}"]`).click();
      await expect.poll(() => page.evaluate(() => getComputedStyle(document.querySelector('#monthly-chart .tg-mline')).stroke)).toBe(mode === 'dark' ? 'rgb(244, 244, 245)' : 'rgb(25, 27, 32)');
    }
    await page.setViewportSize({ width: 390, height: 844 });
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    await page.screenshot({ path: 'test-results/monthly-chart-mobile.png', fullPage: true });
    await page.locator('[data-trend-mode="daily"]').click();
    await expect(page.locator('#daily-chart')).toBeVisible();
    await expect(page.locator('#trend-title')).toHaveText('แนวโน้มยอดขายรายวัน');
    failed = true;
    await page.locator('[data-trend-mode="monthly"]').click();
    await expect(page.locator('#monthly-retry')).toBeVisible();
    assert.equal(await page.evaluate(() => window.salesTrend.debug().monthly), null);
    ...
    assert.deepEqual(await page.evaluate(() => window.salesTrend.debug().monthly.values), [500,...Array(11).fill(null)]);
