...
async function fixture(t){
  ...
  const env={AUTH_USERNAME:'root',AUTH_PASSWORD_HASH:await hashPassword('fixture-password-123'),AUTH_COOKIE_SECURE:'false',AUTH_REQUIRE_2FA:'false'};
...
test('policy-compliant passwords work for creation, login and reset; weak/empty are rejected; empty preserves an edited password',async t=>{
  const f=await fixture(t),root=await f.login('root');
  const body={username:'short-password-user',full_name:'Test',role:'admin',is_active:true,password:'blue-sky42',additionalPermissions:[],territoryIds:[]};
  ...
  assert.equal((await login('blue-sky42')).status,200);
  for(const weak of ['short1','password123','abcdefgh','12345678','aaaaaaa1','x'])assert.equal((await f.request('/api/admin/users/'+user.id+'/password',root.cookie,'POST',{password:weak,currentPassword:'blue-sky42'})).status,400);
  assert.equal((await f.request('/api/admin/users/'+user.id,root.cookie,'PUT',{...body,password:''})).status,200);
  assert.equal((await login('blue-sky42')).status,200);
  assert.equal((await f.request('/api/admin/users/'+user.id,root.cookie,'PUT',{...body,password:'green-tea77',currentPassword:'wrong'})).status,400);
  assert.equal((await f.request('/api/admin/users/'+user.id,root.cookie,'PUT',{...body,password:'green-tea77',currentPassword:'blue-sky42'})).status,200);
  assert.equal((await login('green-tea77')).status,200);assert.equal((await login('blue-sky42')).status,401);
  assert.equal((await f.request('/api/admin/users/'+user.id+'/password',root.cookie,'POST',{password:'กข-tea88',currentPassword:'wrong'})).status,400);
  assert.equal((await f.request('/api/admin/users/'+user.id+'/password',root.cookie,'POST',{password:'กข-tea88',currentPassword:'green-tea77'})).status,200);
  assert.equal((await login('กข-tea88')).status,200);assert.equal((await login('green-tea77')).status,401);
  assert.equal((await f.request('/api/admin/users/'+user.id+'/password',root.cookie,'POST',{password:'กข-tea88'.repeat(25)+'1',currentPassword:'กข-tea88'})).status,400);
});
