...
try {
  ...
  await page.locator('[name=full_name]').fill('ผู้ใช้ทดสอบ');await page.locator('[name=username]').fill('ui-new');await page.locator('[name=password]').fill('ui-fixture-password');await page.locator('[name=role]').selectOption('sales');await page.getByLabel('ภาคกลาง',{exact:true}).check();await page.getByRole('button',{name:'บันทึกผู้ใช้',exact:true}).click();await page.getByRole('cell',{name:'ผู้ใช้ทดสอบ / ui-new',exact:true}).waitFor();
