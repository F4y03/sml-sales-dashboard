(() => {
  ...
  card.type = "button";
  card.className = "card kpi growth-card";
  card.setAttribute("aria-haspopup", "dialog");
  card.setAttribute("aria-controls", "growth-dialog");
  card.innerHTML =
    '<div class="kpi-top"><span>การเติบโต ↗</span><span class="metric-icon">ⓘ</span></div><div class="metric" id="overview-mom">—</div><div class="kpi-foot" id="overview-yoy">กำลังโหลด…</div><div class="kpi-foot">ดูช่วงเวลาเปรียบเทียบ →</div>';
  ...
  const percent = (n) =>
    n == null ? "คำนวณไม่ได้" : `${n > 0 ? "+" : ""}${Number(n).toFixed(2)}%`;
  const money = (n) =>
  ...
  window.resetOverviewGrowth = () => {
    ...
    caption.textContent = "ยังไม่มีข้อมูลการเติบโต";
    dialog.close();
  ...
  window.loadOverviewGrowth = async (start, end) => {
    ...
    try {
      ...
      metric.textContent =
        data.mom == null
          ? "—"
          : `${data.mom > 0 ? "↑ " : data.mom < 0 ? "↓ " : ""}${percent(data.mom)}`;
      metric.dataset.direction =
        data.mom > 0 ? "up" : data.mom < 0 ? "down" : "";
      ...
      for (const [label, net, change, isCurrent, period] of [
        ...
        const value = node(
          ...
          isCurrent
            ...
            : change == null
              ? "—"
              : `${change > 0 ? "↑ " : change < 0 ? "↓ " : ""}${percent(change)}`,
          "growth-comparison-value",
        );
        if (!isCurrent)
          value.dataset.direction =
            change > 0 ? "up" : change < 0 ? "down" : "";
        section.append(
          node("p", label, "growth-comparison-label"),
          node("p", range(period), "growth-comparison-dates"),
          value,
          node(
            "p",
            isCurrent ? "ช่วงวันที่เลือก" : money(net),
      ...
      details.append(
        metrics,
        node("h3", "อ่านตัวเลขเพื่อใช้งาน"),
        node(
          "p",
          "การเติบโต = (ยอดปัจจุบัน − ยอดช่วงเปรียบเทียบ) ÷ ยอดช่วงเปรียบเทียบ × 100 ใช้ MoM ดูแนวโน้มระยะสั้น และ YoY ประกอบการดูฤดูกาล",
        ),
      );
      ...
      note.append(
        ...
        node(
          "p",
          "เลื่อนวันที่ย้อนหลังตามเดือนและปี โดยปรับวันที่เกินสิ้นเดือนเป็นวันสุดท้ายของเดือน จำนวนวันอาจต่างกัน ไม่คำนวณ % เมื่อยอดฐานเป็นศูนย์หรือติดลบ",
        ),
        node(
          "p",
          "ใช้ยอดขายสุทธิ = ขาย + เพิ่มหนี้ − รับคืน/ลดหนี้ ตามวันที่เอกสาร ต่างจากยอดขายก่อนหักคืนในการ์ดข้างเคียง",
        ),
      );
      details.append(note);
      card.disabled = false;
      ...
      if (controller === active) {
        metric.textContent = "—";
        caption.textContent = "โหลดการเติบโตไม่สำเร็จ";
      }
    } finally {
