...
async function run(page=0,silent=false){if(!selected||!snapshot)return;const version=++requestVersion;const key=selected.key;controller?.abort();controller=new AbortController();const activeController=controller;const timer=setTimeout(()=>activeController.abort(),35000);$('run-report').disabled=true;$('previous-page').disabled=true;$('next-page').disabled=true;if(!silent)clearResult();if(!silent)status('กำลังดึงรายงานจาก SML…');try{const data=await json(`/api/reports/${key}/run`,{method:'POST',headers:{'Content-Type':'application/json','X-PRPlus-Request':'1'},body:JSON.stringify({...snapshot,page,pageSize:100}),signal:controller.signal});if(version!==requestVersion)return;result=data;render(data);status('โหลดข้อมูลจริงจาก SML สำเร็จ · ไม่มีการแก้ไขข้อมูลต้นฉบับ');}catch(e){if(version===requestVersion)status(e.name==='AbortError'?'การโหลดถูกยกเลิกหรือใช้เวลานานเกินไป กรุณาจำกัดช่วงวันที่/รหัสสินค้า':e.message,true);}finally{clearTimeout(timer);if(version===requestVersion)$('run-report').disabled=false;}}
...
setInterval(() => {
  ...
  if (selected && snapshot) { if (!$('run-report').disabled) run(result?.page || 0, true); }
