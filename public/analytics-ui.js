(() => {
  ...
  function draw(d){
    ...
    const barPalette=['#813437','#a34b4e','#bb7261','#b88935','#cc9589','#c8a46a','#d9b7a6','#f5dcd7','#e8c9bf','#eedbd2'];
    chart=new Chart(el('analysis-chart'),{type:timeline?'line':'bar',data:{labels,datasets:[{data:shown.map(r=>r.value),borderColor:'#813437',backgroundColor:timeline?'rgba(129, 52, 55,0.08)':shown.map((r,i)=>Number(r.value)<0?'#c34f52':barPalette[i%barPalette.length]),borderRadius:timeline?0:6,borderWidth:timeline?2.5:0,borderSkipped:false,pointRadius:timeline?3:0,pointBackgroundColor:'#fff',pointBorderColor:'#813437',pointBorderWidth:2,pointHoverRadius:6,pointHoverBackgroundColor:'#813437',pointHoverBorderColor:'#fff',pointHoverBorderWidth:3,tension:0,fill:timeline,maxBarThickness:24,barPercentage:0.75,categoryPercentage:0.82}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:400,easing:'easeOutQuart'},indexAxis:timeline?'x':'y',layout:{padding:{top:4,right:timeline?4:16,left:4}},plugins:{legend:{display:false},tooltip:{backgroundColor:'#482629',titleColor:'#f5dcd7',bodyColor:'#fff',padding:{top:10,bottom:10,left:14,right:14},cornerRadius:10,displayColors:false,titleFont:{size:11,weight:'500'},bodyFont:{size:13,weight:'600'},callbacks:{title:items=>labels[items[0].dataIndex],label:ctx=>`${isQty?'':'฿'}${format.format(timeline?ctx.parsed.y:ctx.parsed.x)}${isQty?' '+shown[ctx.dataIndex].unit:''}`}}},scales:timeline?{x:{grid:{display:false},border:{display:false},ticks:{maxTicksLimit:6,maxRotation:0,font:{size:10,weight:'500'},color:'#716b68',padding:6}},y:{beginAtZero:true,border:{display:false},grid:{color:'#faf7f5',lineWidth:1},ticks:{font:{size:10,weight:'500'},color:'#716b68',padding:8}}}:{x:{beginAtZero:true,border:{display:false},grid:{color:'#faf7f5',lineWidth:1},ticks:{maxTicksLimit:4,font:{size:10,weight:'500'},color:'#716b68',padding:4,callback:v=>Math.abs(v)>=1000000?v/1000000+'m':Math.abs(v)>=1000?v/1000+'k':v}},y:{grid:{display:false},border:{display:false},ticks:{autoSkip:false,font:{size:11,weight:'500'},color:'#716b68',padding:8,callback:(_,i)=>labels[i]?.length>22?labels[i].slice(0,22)+'…':labels[i]}}}}});
