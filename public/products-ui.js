...
async function load(page=0,filters=applied,silent=false){
  ...
  try{
    ...
    for(const p of data.rows){const tr=document.createElement('tr');for(const v of [p.code,p.name_1,p.group_main_name||p.group_main,p.unit_standard,p.item_brand,priceLabel(p.catalog_sale_price)]){const td=document.createElement('td');td.textContent=v||'—';tr.append(td);}const td=document.createElement('td'),b=document.createElement('button');b.type='button';b.className='button secondary';b.textContent='ดูรายละเอียด';b.onclick=()=>detail(p);td.append(b);tr.append(td);$('product-table').append(tr);}
    if(!data.rows.length){const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=7;td.textContent='ไม่พบสินค้าตามตัวกรอง';tr.append(td);$('product-table').append(tr);}
